...
export function generateGcodeFromForm(formState) {
  ...
  g.push("; Generated by retraction_doe_generator")
  g.push("; ========================================")
  g.push(";")
  g.push("; === Printer Settings ===")
  g.push(`; Bed size: ${cfg.BED_SIZE_X} x ${cfg.BED_SIZE_Y} mm`)
  g.push(`; Bed temperature: ${cfg.BED_TEMP}°C`)
  g.push(`; Hotend temperature: ${cfg.HOTEND_TEMP}°C`)
  g.push(`; Filament diameter: ${cfg.FILAMENT_DIAMETER} mm`)
  g.push(";")
  g.push("; === Retraction Test Parameters ===")
  g.push(`; Retraction distance (min): ${cfg.MIN_RETRACT_DIST} mm`)
  g.push(`; Retraction distance (max): ${cfg.MAX_RETRACT_DIST} mm`)
  g.push(`; Retraction speed (min): ${cfg.MIN_RETRACT_SPEED} mm/s`)
  g.push(`; Retraction speed (max): ${cfg.MAX_RETRACT_SPEED} mm/s`)
  g.push(`; DOE grid: ${cfg.ROWS} rows x ${cfg.COLS} columns`)
  g.push(`; Lines per parameter: ${cfg.LINES_PER_PARAM}`)
  g.push(";")
  g.push("; === Speed & Acceleration ===")
  g.push(`; First layer speed: ${cfg.FIRST_LAYER_SPEED} mm/s`)
  g.push(`; Print speed: ${cfg.PRINT_SPEED} mm/s`)
  g.push(`; Travel speed: ${cfg.TRAVEL_SPEED} mm/s`)
  g.push(`; Text speed: ${cfg.TEXT_SPEED} mm/s`)
  g.push(`; Print acceleration: ${cfg.PRINT_ACCEL} mm/s²`)
  g.push(`; Retract acceleration: ${cfg.RETRACT_ACCEL} mm/s²`)
  g.push(`; Travel acceleration: ${cfg.TRAVEL_ACCEL} mm/s²`)
  if (Number.isFinite(cfg.FIRST_LAYER_PRINT_ACCEL)) {
    g.push(`; First layer print acceleration: ${cfg.FIRST_LAYER_PRINT_ACCEL} mm/s²`)
  }
  g.push(";")
  g.push("; === Layer Settings ===")
  g.push(`; First layer Z: ${cfg.FIRST_LAYER_Z} mm`)
  g.push(`; Layer height: ${cfg.LAYER_HEIGHT} mm`)
  g.push(`; Number of DOE layers: ${cfg.NUM_DOE_LAYERS}`)
  g.push(`; Z-hop: ${cfg.Z_HOP} mm`)
  g.push(";")
  g.push("; === Extrusion Settings ===")
  g.push(`; Extrusion width: ${cfg.EXTRUSION_WIDTH} mm`)
  g.push(`; Flow factor: ${cfg.FLOW_FACTOR.toFixed(3)}`)
  if (Number.isFinite(cfg.PRESSURE_ADVANCE)) {
    g.push(`; Pressure advance: ${cfg.PRESSURE_ADVANCE}`)
  }
  g.push(";")
  g.push("; === Fan Settings ===")
  g.push(`; First layer fan speed: ${cfg.FIRST_LAYER_FAN_SPEED}%`)
  g.push(`; Normal fan speed: ${cfg.FAN_SPEED}%`)
  g.push(";")
  g.push("; === Pattern Geometry ===")
  g.push(`; Segment length: ${cfg.SEGMENT_LENGTH} mm`)
  g.push(`; Travel length: ${cfg.TRAVEL_LENGTH} mm`)
  g.push(`; Row gap: ${cfg.ROW_GAP} mm`)
  g.push(`; Margin: ${cfg.MARGIN} mm`)
  g.push(`; Distance label padding: ${cfg.DIST_LABEL_PAD} mm`)
  g.push(`; Row font size: ${cfg.ROW_FONT_SIZE} mm`)
  g.push(`; Speed font size: ${cfg.SPEED_FONT_SIZE} mm`)
  g.push(";")
  g.push("; ========================================")
  g.push("")
  ...
  if (cfg.NUM_DOE_LAYERS >= 1) {
    ...
    g.push(g1Move({ z: zLabels, f: cfg.TRAVEL_SPEED * 60.0, bedX: cfg.BED_SIZE_X, bedY: cfg.BED_SIZE_Y }))
    labelPattern(g, cfg, {
      originX, originY,
      baseX0, baseY0,
      rowBlockHeight,
